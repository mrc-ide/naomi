...
test_that("get_area_collection() defaults to all areas at lowest level", {
  val <- get_area_collection(mwi_areas)
...
test_that("get_area_collection() returns correct collection at each level", {
  val1 <- get_area_collection(mwi_areas, level = 1)
  val3 <- get_area_collection(mwi_areas, level = 3)
...
test_that("get_area_collection() throws error if area_level outside of hierarchy range", {
  expect_error(get_area_collection(mwi_areas, level = -1))
  expect_error(get_area_collection(mwi_areas, level = 6))
  expect_error(get_area_collection(mwi_areas, level = 3.3))
  expect_error(get_area_collection(mwi_areas, level = "foo"))
})

test_that("get_area_collection() throws error if area_scope is not in area_id", {
  expect_error(get_area_collection(mwi_areas, area_scope = "MWI.5.3"))
  expect_error(get_area_collection(mwi_areas, area_scope = "foo"))
  expect_error(get_area_collection(mwi_areas, area_scope = 3))
})

test_that("get_area_collection() returns nothing if area_scope is lower level than area_level", {
  val <- get_area_collection(mwi_areas, level = 2, area_scope = "MWI.3.5.24")
