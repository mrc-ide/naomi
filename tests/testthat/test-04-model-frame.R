test_that("get_age_group_out() returns expected groups", {
  ...
  expect_setequal(
    get_age_group_out(c("Y015_049", "Y050_064", "Y065_999")),
...
test_that("artnum_mf() returns expected number of records", {
  ...
  expect_equal(nrow(mf$model_input), 0L)
  mf <- artnum_mf(NULL, demo_art_number, a_naomi_mf)
  expect_named(mf$model_input, c("area_id", "sex", "age_group", "art_current"))
...
test_that("artnum_mf() throws errors for invalid inputs", {
  expect_error(
    artnum_mf("CY1924Q4", demo_art_number, a_naomi_mf),
...
test_that("artnum_mf() gives informative error for duplicate ART records", {
  dup_row <- demo_art_number[demo_art_number$calendar_quarter == "CY2017Q4", ][
    1,
  ]
  art_with_dups <- dplyr::bind_rows(demo_art_number, dup_row)

  expect_error(
    artnum_mf("CY2017Q4", art_with_dups, a_naomi_mf),
    "Duplicated records found in ART data for:"
  )
  expect_error(
    artnum_mf("CY2017Q4", art_with_dups, a_naomi_mf),
...
test_that("artnum_mf() works with single quarter ART data", {
  data_art_single_quarter <- dplyr::filter(
    demo_art_number,
...
test_that("artnum_mf() selects adjusted number on ART", {
  ...
  expect_true(all(unlist(
    dat_015plus$art_current != dat_015plus$art_current_input
  )))
})


test_that("population calibration options", {
  mf_none <- naomi_model_frame(
    a_area_merged,
  ...
  expect_equal(
    mf_none$spectrum_calibration$population_raw,
    mf_none$spectrum_calibration$population_calibrated
  )
  expect_false(all(
    mf_none$spectrum_calibration$population_spectrum ==
      mf_none$spectrum_calibration$population_calibrated
  ))
  expect_equal(
    sum(
      mf_none$mf_model$population_t1 +
  ...
  expect_false(
    sum(mf_nat$spectrum_calibration$population_raw) ==
  ...
  expect_false(
    sum(mf_subnat$spectrum_calibration$population_raw) ==
      sum(mf_subnat$spectrum_calibration$population_calibrated)
  )
  expect_equal(
    mf_subnat$spectrum_calibration$population_spectrum,
    mf_subnat$spectrum_calibration$population_calibrated
  )
  expect_equal(
    sum(
      mf_subnat$mf_model$population_t1 +
...
test_that("naomi_model_frame() errors with clear message when population level mismatches model level", {
  mf_args <- list(
    a_area_merged,
    demo_population_agesex,
    a_spec,
  ...
  expect_error(
    do.call(naomi_model_frame, mf_args),
    "Model options set to generate Region level estimates but population data provided at District \\+ Metro level\\."
  )
})
...
test_that("use_kish returns Kish effective sample size", {
  ...
  expect_true(all(
    mf_kish$model_input$n_eff < mf_srs$model_input$n_eff |
      mf_srs$model_input$n_eff == 1
  ))
  expect_equal(
    mf_kish_scaled$model_input$n_eff * 2.5,
    mf_kish$model_input$n_eff
  )

  expect_equal(mf_default$model_input$x_eff, mf_kish$model_input$x_eff)
  expect_true(all(
    mf_kish$model_input$x_eff < mf_srs$model_input$x_eff |
      mf_srs$model_input$x_eff == 0
  ))
  expect_equal(
    mf_kish_scaled$model_input$x_eff * 2.5,
    mf_kish$model_input$x_eff
  )
})

test_that("select_naomi_data() returns expected stratifications", {
  prev_age_groups <- dplyr::filter(
    get_age_groups(),
  ...
  artcov_age_groups <- dplyr::filter(
    get_age_groups(),
    age_group %in% get_five_year_age_groups(),
    age_group_start + age_group_span - 1 < 65
  ) %>%
  ...
  recent_age_groups <- dplyr::filter(
    get_age_groups(),
    age_group %in% get_five_year_age_groups(),
    age_group_start >= 15,
    age_group_start + age_group_span - 1 < 65
...
test_that("survey_mf(..., use_aggregate) option returns expected results", {
  ...
  aggregate_survey_good <- dplyr::filter(
    aggregate_survey,
...
test_that("naomi_model_frame() interpolated population depends on quarter specification", {
  areas_zone <- read_area_merged(system_file(
    "extdata/demo-subnational-pjnz/demo_areas_region-pjnz.geojson"
  ))
  pop_zone <- read_population(system_file(
    "extdata/demo-subnational-pjnz/demo_population_zone.csv"
  ))

  ## Test files created with Spectrum v5.87 -- mid-year population projection
  pjnz_old <- system_file(
    "extdata/demo-subnational-pjnz/demo_mwi2019_region-pjnz.zip"
  )
  spec_old <- extract_pjnz_naomi(pjnz_old)

  expect_true(all(spec_old$quarter == 2))

  mf_old <- naomi_model_frame(
    areas_zone,
    pop_zone,
    spec_old,
    scope = "MWI",
  ...
  expect_equal(
    sum(dplyr::filter(spec_old, year == 2019)$totpop),
    sum(mf_old$mf_model$population_t3)
  )
  expect_equal(
    sum(dplyr::filter(spec_old, year == 2019)$hivpop),
    sum(
      dplyr::filter(
        mf_old$spectrum_calibration,
        calendar_quarter == "CY2019Q2"
      )$plhiv_spectrum
    )
  )
  expect_equal(
    sum(dplyr::filter(spec_old, year == 2019)$artpop),
    sum(
      dplyr::filter(
        mf_old$spectrum_calibration,
        calendar_quarter == "CY2019Q2"
  ...
  pjnz_new <- system_file(
    "extdata/demo-subnational-pjnz/demo_mwi2019_region-pjnz_v6.2.zip"
  )
  spec_new <- extract_pjnz_naomi(pjnz_new)

  expect_true(all(spec_new$quarter == 4))

  mf_new <- naomi_model_frame(
    areas_zone,
    pop_zone,
    spec_new,
    scope = "MWI",
  ...
  expect_equal(
    sum(dplyr::filter(spec_new, year == 2018)$totpop),
    sum(mf_new$mf_model$population_t2)
  )
  expect_equal(
    sum(dplyr::filter(spec_new, year == 2018)$hivpop),
    sum(
      dplyr::filter(
        mf_new$spectrum_calibration,
  ...
  expect_equal(
    sum(dplyr::filter(spec_new, year == 2018)$artpop_dec31),
    sum(
      dplyr::filter(
        mf_new$spectrum_calibration,
        calendar_quarter == "CY2018Q4"
  ...
  ## Outputs should match for T2 (CY2018Q4 -- mid-year)
  expect_equal(
    sum(dplyr::filter(spec_alt, year == 2018)$totpop),
    sum(mf_alt$mf_model$population_t2)
  )
  expect_equal(
    sum(dplyr::filter(spec_alt, year == 2018)$hivpop),
    sum(
      dplyr::filter(
        mf_alt$spectrum_calibration,
        calendar_quarter == "CY2018Q4"
      )$plhiv_spectrum
    )
  )
  ...
  expect_equal(
    sum(dplyr::filter(spec_alt, year == 2018)$artpop),
    sum(
      dplyr::filter(
        mf_alt$spectrum_calibration,
        calendar_quarter == "CY2018Q4"
