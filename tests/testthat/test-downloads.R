...
test_that("spectrum download works with old data - v 2.3.15", {
  out <- hintr_prepare_spectrum_download(a_hintr_output_2.3.15)
  expect_true(file.exists(out$path))
...
test_that("coarse output download works with old data - v 2.3.15", {
  out <- hintr_prepare_coarse_age_group_download(a_hintr_output_2.3.15)
  expect_true(file.exists(out$path))
...
test_that("summary report download works with old data - v 2.3.15", {
  testthat::skip("TODO: this should work after summary report update")
  out <- hintr_prepare_summary_report_download(a_hintr_output_2.3.15)
  expect_true(file.exists(out$path))
