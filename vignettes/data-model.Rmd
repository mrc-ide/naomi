...
```{r create_model, include = FALSE,  message = FALSE}
dm_add_colors(
  dm_color_scheme(
...
dm <-
  ...
  dm_set_key("fertility", c("area_id", "year", "age_group")) %>%
  ...
  dm_set_key("art_number", c("area_id", "sex", "age_group", "year")) %>%
  dm_set_key("anc_testing", c("area_id", "age_group", "year")) %>%
  dm_add_references(
...
```{r create_graph, fig.width = 7, fig.height = 10, fig.align = "center", echo = FALSE}
#' Render graph
...
* `population_agesex` contains population estimates by area, sex, and five-year age
   group for years spanning the range of time points estimated by the model. 
   Estimates are required at the highest level of the area hierarchy for 
   all age groups from 0-4 through 80+. 
* `fertility` contains age-specific fertility rate (ASFR) estimates by area.
   
Time is identified as `calendar_quarter` coded as a string for example "CY2019Q2" referencing mid-year population estimates for 2019.
...
* `art_number` reports the number currently receiving ART at the end of each year by area.
* `anc_testing` reports antenatal clinic (ANC) attendees and outcomes during the year. 
