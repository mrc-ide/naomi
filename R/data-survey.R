...
expand_survey_clusters <- function(survey_clusters,
  ...
  clusters <- survey_clusters %>%
    select(survey_id, cluster_id, restype, survey_region_id, geoloc_area_id) %>%
    left_join(
      survey_regions %>%
        select(survey_id, survey_region_id, survey_region_area_id),
  ...
  val <- clusters %>% filter(between(area_level, top_level, bottom_level))
  ...
  while(any(clusters$area_level >= top_level)) {
    ...
    val <- bind_rows(
      val,
      clusters %>%
        filter(between(area_level, top_level, bottom_level))
    )
  }
...
calc_survey_hiv_indicators <- function(survey_meta,
  ...
  if(!is.null(age_group_id))
    age_group <- filter(age_group, age_group_id %in% !!age_group_id)

  sex_age_group <- crossing(sex, age_group)
  ...
  sex_age_group <- survey_meta %>%
    select(survey_id, female_age_min, female_age_max,
           male_age_min, male_age_max) %>%
    crossing(sex_age_group) %>%
    filter(age_group_start >= case_when(sex == "male" ~ male_age_min,
           ...
           age_group_start + age_group_span <= case_when(sex == "male" ~ male_age_max,
                                                 ...
                                                 sex == "both" ~ pmin(male_age_max,
                                                                      female_age_max)) + 1) %>%
    select(survey_id, sex, age_group_id, age_group_label, age_group_start, age_group_span)


  ## 2. Expand clusters to identify all clusters within each area

  clust <- filter(survey_clusters, survey_id %in% survey_meta$survey_id)
  ...
  ind <- survey_individuals %>%
    inner_join(survey_biomarker,
               by = c("survey_id", "cluster_id", "household", "line")) %>%
    filter(survey_id %in% survey_meta$survey_id,
           !is.na(hivstatus)) %>%
    select(survey_id, cluster_id, sex, age, hivweight, hivstatus, artself, arv, vls, recent) %>%
    bind_rows({.} %>% mutate(sex = "both")) %>%
    inner_join(sex_age_group, by = c("survey_id", "sex")) %>%
    filter(age >= age_group_start,
           age < age_group_start + age_group_span)

  ## 4. Join expanded age/sex and expanded cluster area map

  ind <- inner_join(ind, clust_area, by = c("survey_id", "cluster_id"))
  ...
  ind <- ind %>%
    group_by(survey_id) %>%
    mutate(has_artcov = any(!is.na(arv) | !is.na(artself)),
           artcov = case_when(!has_artcov ~ NA_integer_,
                              hivstatus == 0 ~ NA_integer_,
                              arv == 1 | artself == 1 ~ 1L,
                              TRUE ~ 0L)) %>%
    select(-has_artcov, -artself, -arv) %>%
    rename(prev = hivstatus) %>%
    gather(indicator, est, prev, artcov, vls, recent) %>%
    filter(!is.na(est))
  ...
  dat <- ind %>%
    filter(!is.na(hivweight), hivweight > 0)

  cnt <- dat %>%
    group_by(indicator, survey_id, area_id, sex, age_group_id) %>%
    summarise(n_cluster = n_distinct(cluster_id),
              n_obs = n()) %>%
    ungroup

  datspl <- dat %>%
    mutate(spl = paste(indicator, survey_id, area_level, sex, age_group_id)) %>%
    split(.$spl)
