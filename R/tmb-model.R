...
prepare_tmb_inputs <- function(naomi_data,
  ...
  dtmb <- list(
    ...
    ##
    ## ART sector model
    X_art_medaid = model.matrix(~female_15plus + age_below15, df),
    X_art_cashpay = model.matrix(~female_15plus + age_below15, df),
    Z_art_medaid_x = sparse_model_matrix(~0 + area_idf, df),
    Z_art_cashpay_x = sparse_model_matrix(~0 + area_idf, df),
    Z_art_medaid_xs = sparse_model_matrix(~0 + area_idf, df, "female_15plus", TRUE),
    Z_art_cashpay_xs = sparse_model_matrix(~0 + area_idf, df, "female_15plus", TRUE),
    Z_art_medaid_xa = sparse_model_matrix(~0 + area_idf, df, "age_below15"),
    Z_art_cashpay_xa = sparse_model_matrix(~0 + area_idf, df, "age_below15"),
    ##
    X_art_medaid_t1t2 = model.matrix(~female_15plus + age_below15, df),
    X_art_cashpay_t1t2 = model.matrix(~female_15plus + age_below15, df),
    Z_art_medaid_x_t1t2 = sparse_model_matrix(~0 + area_idf, df),
    Z_art_cashpay_x_t1t2 = sparse_model_matrix(~0 + area_idf, df),
    Z_art_medaid_xs_t1t2 = sparse_model_matrix(~0 + area_idf, df, "female_15plus", TRUE),
    Z_art_cashpay_xs_t1t2 = sparse_model_matrix(~0 + area_idf, df, "female_15plus", TRUE),
    Z_art_medaid_xa_t1t2 = sparse_model_matrix(~0 + area_idf, df, "age_below15"),
    Z_art_cashpay_xa_t1t2 = sparse_model_matrix(~0 + area_idf, df, "age_below15"),
    ##
    X_art_medaid_t2t3 = model.matrix(~female_15plus + age_below15, df),
    X_art_cashpay_t2t3 = model.matrix(~female_15plus + age_below15, df),
    Z_art_medaid_x_t2t3 = sparse_model_matrix(~0 + area_idf, df),
    Z_art_cashpay_x_t2t3 = sparse_model_matrix(~0 + area_idf, df),
    Z_art_medaid_xs_t2t3 = sparse_model_matrix(~0 + area_idf, df, "female_15plus", TRUE),
    Z_art_cashpay_xs_t2t3 = sparse_model_matrix(~0 + area_idf, df, "female_15plus", TRUE),
    Z_art_medaid_xa_t2t3 = sparse_model_matrix(~0 + area_idf, df, "age_below15"),
    Z_art_cashpay_xa_t2t3 = sparse_model_matrix(~0 + area_idf, df, "age_below15"),    
    ##
    ...
    x_artnum_public_t1 = naomi_data$artnum_t1_dat$art_current_public,
    x_artnum_medaid_t1 = naomi_data$artnum_t1_dat$art_current_medaid,
    x_artnum_cashpay_t1 = naomi_data$artnum_t1_dat$art_current_cashpay,
    A_artattend_t2 = A_artattend_t2,
    x_artnum_t2 = naomi_data$artnum_t2_dat$art_current,
    x_artnum_public_t2 = naomi_data$artnum_t2_dat$art_current_public,
    x_artnum_medaid_t2 = naomi_data$artnum_t2_dat$art_current_medaid,
    x_artnum_cashpay_t2 = naomi_data$artnum_t2_dat$art_current_cashpay,    
    A_artattend_t3 = A_artattend_t3,
    x_artnum_t3 = naomi_data$artnum_t3_dat$art_current,
    x_artnum_public_t3 = naomi_data$artnum_t3_dat$art_current_public,
    x_artnum_medaid_t3 = naomi_data$artnum_t3_dat$art_current_medaid,
    x_artnum_cashpay_t3 = naomi_data$artnum_t3_dat$art_current_cashpay,    
  ...
  ptmb <- list(
    ...
    ##
    ## ART sector model
    log_foreign_art_eta_scalar = 0.0,
    ##
    beta_art_medaid = numeric(ncol(dtmb$X_art_medaid)),
    beta_art_cashpay = numeric(ncol(dtmb$X_art_cashpay)),
    u_art_medaid_x = numeric(ncol(dtmb$Z_art_medaid_x)),
    u_art_cashpay_x = numeric(ncol(dtmb$Z_art_cashpay_x)),
    u_art_medaid_xs = numeric(ncol(dtmb$Z_art_medaid_xs)),
    u_art_cashpay_xs = numeric(ncol(dtmb$Z_art_cashpay_xs)),
    u_art_medaid_xa = numeric(ncol(dtmb$Z_art_medaid_xa)),
    u_art_cashpay_xa = numeric(ncol(dtmb$Z_art_cashpay_xa)),
    log_sigma_art_medaid_x = log(2.5),
    log_sigma_art_cashpay_x = log(2.5),
    log_sigma_art_medaid_xs = log(2.5),
    log_sigma_art_cashpay_xs = log(2.5),
    log_sigma_art_medaid_xa = log(2.5),
    log_sigma_art_cashpay_xa = log(2.5),
    ## 
    beta_art_medaid_t1t2 = numeric(ncol(dtmb$X_art_medaid_t1t2)),
    beta_art_cashpay_t1t2 = numeric(ncol(dtmb$X_art_cashpay_t1t2)),
    u_art_medaid_x_t1t2 = numeric(ncol(dtmb$Z_art_medaid_x_t1t2)),
    u_art_cashpay_x_t1t2 = numeric(ncol(dtmb$Z_art_cashpay_x_t1t2)),
    u_art_medaid_xs_t1t2 = numeric(ncol(dtmb$Z_art_medaid_xs_t1t2)),
    u_art_cashpay_xs_t1t2 = numeric(ncol(dtmb$Z_art_cashpay_xs_t1t2)),
    u_art_medaid_xa_t1t2 = numeric(ncol(dtmb$Z_art_medaid_xa_t1t2)),
    u_art_cashpay_xa_t1t2 = numeric(ncol(dtmb$Z_art_cashpay_xa_t1t2)),
    log_sigma_art_medaid_xt = log(2.5),
    log_sigma_art_cashpay_xt = log(2.5),
    log_sigma_art_medaid_xst = log(2.5),
    log_sigma_art_cashpay_xst = log(2.5),
    log_sigma_art_medaid_xat = log(2.5),
    log_sigma_art_cashpay_xat = log(2.5),
    ## 
    beta_art_medaid_t2t3 = numeric(ncol(dtmb$X_art_medaid_t2t3)),
    beta_art_cashpay_t2t3 = numeric(ncol(dtmb$X_art_cashpay_t2t3)),
    u_art_medaid_x_t2t3 = numeric(ncol(dtmb$Z_art_medaid_x_t2t3)),
    u_art_cashpay_x_t2t3 = numeric(ncol(dtmb$Z_art_cashpay_x_t2t3)),
    u_art_medaid_xs_t2t3 = numeric(ncol(dtmb$Z_art_medaid_xs_t2t3)),
    u_art_cashpay_xs_t2t3 = numeric(ncol(dtmb$Z_art_cashpay_xs_t2t3)),
    u_art_medaid_xa_t2t3 = numeric(ncol(dtmb$Z_art_medaid_xa_t2t3)),
    u_art_cashpay_xa_t2t3 = numeric(ncol(dtmb$Z_art_cashpay_xa_t2t3))
  )
...
make_tmb_obj <- function(data, par, calc_outputs = 1L, inner_verbose = FALSE,
  ...
  obj <- TMB::MakeADFun(data = data,
                        ...
                        map = list(
                          ...
                          ## ,
                          ## ## 
                          ## beta_art_medaid = factor(rep(NA, length(par$beta_art_medaid))),
                          ## beta_art_medaid_t1t2 = factor(rep(NA, length(par$beta_art_medaid_t1t2))),
                          ## beta_art_medaid_t2t3 = factor(rep(NA, length(par$beta_art_medaid_t2t3))),
                          ## beta_art_cashpay = factor(rep(NA, length(par$beta_art_cashpay))),
                          ## beta_art_cashpay_t1t2 = factor(rep(NA, length(par$beta_art_cashpay_t1t2))),
                          ## beta_art_cashpay_t2t3 = factor(rep(NA, length(par$beta_art_cashpay_t2t3))),
                          ## ##
                          ## u_art_medaid_x = factor(rep(NA, length(par$u_art_medaid_x))),
                          ## u_art_cashpay_x = factor(rep(NA, length(par$u_art_cashpay_x))),
                          ## log_sigma_art_medaid_x = factor(NA),
                          ## log_sigma_art_cashpay_x = factor(NA),
                          ## u_art_medaid_x = factor(rep(NA, length(par$u_art_medaid_x))),
                          ## u_art_cashpay_x = factor(rep(NA, length(par$u_art_cashpay_x))),
                          ## log_sigma_art_medaid_x = factor(NA),
                          ## log_sigma_art_cashpay_x = factor(NA),                          
                          ## ##
                          ## u_art_medaid_xs = factor(rep(NA, length(par$u_art_medaid_xs))),
                          ## u_art_medaid_xa = factor(rep(NA, length(par$u_art_medaid_xa))),
                          ## u_art_cashpay_xs = factor(rep(NA, length(par$u_art_cashpay_xs))),
                          ## u_art_cashpay_xa = factor(rep(NA, length(par$u_art_cashpay_xa))),
                          ## log_sigma_art_medaid_xs = factor(NA),
                          ## log_sigma_art_medaid_xa = factor(NA),
                          ## log_sigma_art_cashpay_xs = factor(NA),
                          ## log_sigma_art_cashpay_xa = factor(NA),
                          ## ##
                          ## u_art_medaid_xs_t1t2 = factor(rep(NA, length(par$u_art_medaid_xs_t1t2))),
                          ## u_art_medaid_xa_t1t2 = factor(rep(NA, length(par$u_art_medaid_xa_t1t2))),
                          ## u_art_cashpay_xs_t1t2 = factor(rep(NA, length(par$u_art_cashpay_xs_t1t2))),
                          ## u_art_cashpay_xa_t1t2 = factor(rep(NA, length(par$u_art_cashpay_xa_t1t2))),
                          ## log_sigma_art_medaid_xst = factor(NA),
                          ## log_sigma_art_medaid_xat = factor(NA),
                          ## log_sigma_art_cashpay_xst = factor(NA),
                          ## log_sigma_art_cashpay_xat = factor(NA),
                          ## ##
                          ## u_art_medaid_xs_t2t3 = factor(rep(NA, length(par$u_art_medaid_xs_t2t3))),
                          ## u_art_medaid_xa_t2t3 = factor(rep(NA, length(par$u_art_medaid_xa_t2t3))),
                          ## u_art_cashpay_xs_t2t3 = factor(rep(NA, length(par$u_art_cashpay_xs_t2t3))),
                          ## u_art_cashpay_xa_t2t3 = factor(rep(NA, length(par$u_art_cashpay_xa_t2t3)))
                        ),
                        random = c("beta_rho",
                                   ...
                                   ##
                                   "log_foreign_art_eta_scalar",
                                   "beta_art_medaid", "u_art_medaid_x", "u_art_medaid_xs", "u_art_medaid_xa",
                                   "beta_art_cashpay", "u_art_cashpay_x", "u_art_cashpay_xs", "u_art_cashpay_xa",
                                   "beta_art_medaid_t1t2", "u_art_medaid_x_t1t2", "u_art_medaid_xs_t1t2", "u_art_medaid_xa_t1t2",
                                   "beta_art_cashpay_t1t2", "u_art_cashpay_x_t1t2", "u_art_cashpay_xs_t1t2", "u_art_cashpay_xa_t1t2",
                                   "beta_art_medaid_t2t3", "u_art_medaid_x_t2t3", "u_art_medaid_xs_t2t3", "u_art_medaid_xa_t2t3",
                                   "beta_art_cashpay_t2t3", "u_art_cashpay_x_t2t3", "u_art_cashpay_xs_t2t3", "u_art_cashpay_xa_t2t3"                                   
                                   ))
