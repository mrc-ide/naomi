...
# STI ART programme data included
sti_art_options = list(options$sti_art_coverage_year1, options$sti_art_coverage_year2, options$sti_art_coverage_year3) 
sti_art_options <- sti_art_options[!sapply(sti_art_options,is.null)]

if(length(sti_art_options)) {
  ## sti_art <- paste0(sti_art_options, sep = "", collapse = collapse_and)
  sti_art <- paste0(sti_art_options, sep = "", collapse = ", ")
} else {
  sti_art <- NA
}
...
**This report uses a customised version of the Naomi model developed for Malawi HIV estimates.** The main customisations are: 
  ...
  * The model useas **ART coverage among HIV positive STI clients** as an additional proxy for district-level ART coverage trends, similar to use of ART coverage among HIV positive pregnant women in the main Naomi model
...
programme_data <- c(art_year, anc_prev, anc_art, sti_art)
...
if (sum(is.na(programme_data)) <3 ) {
  ...
  text2 <- tibble::tibble(prefix = c("National programme data on ART coverage for: ",
                                  ...
                                  "National programme data on _STI client ART coverage_ for: "
                                  ), 
                       source = c(art_year,
                                  anc_prev, 
                                  anc_art,
...
p15 <- district_barplot(naomi_output = outputs, 
                        ...
                        colour_palette  = "blue",
                        x_title = t_("INCIDENCE_DISTRICT_BARPLOT_X_AXIS_LABEL"), 
