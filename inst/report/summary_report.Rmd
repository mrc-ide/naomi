...
```{r include = print_en, eval = print_en}
plot(1:5, 1:5)
```

```{r include = print_fr, eval = print_fr}
plot(5:1, 5:1)
```
...
if(length(anc_art_options)) {
  anc_art <- paste0(anc_art_options, sep = "", collapse = " and ")
} else {
  anc_art <- NA
}
...
if(length(anc_prev_options)) {
  anc_prev <- paste0(anc_prev_options, sep = "", collapse = " and ")
} else {
  anc_prev <- NA
}
...
text <- tibble::tibble(prefix = c("Étalonnage de la population: ",
                       ...
                       source = c(calibration_options$spectrum_population_calibration,
                                  ...
                                  calibration_options$spectrum_infections_calibration_level,
                                  calibration_options$spectrum_infections_calibration_strat,
                                  calibration_options$calibrate_method
                                  ))
...
p1 <- map_outputs(geom_data = data, 
                  ...
                  fig_title = "Personnes vivant avec le VIH (15 ans et plus)",
                  legend_title = "PVVIH")
...
`r if(print_fr){"_Données d'enquête_"}`
...
`r if(print_fr){"_Couverture TARV_"}`
...
`r if(print_en){"_Current estimates and one-year ahead projection_"}`
