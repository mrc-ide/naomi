...
options <- yaml::yaml.load_file(options)
...
map_outputs <- function(geom_data,
  ...
  ggplot2::ggplot(fig_data, ggplot2::aes(fill = mean)) +
    ...
    ggplot2::theme(legend.position = "bottom",
                   ...
                   plot.margin = margin(0, 0.4, 0, 0.4, "cm")) +
...
pop_pyramid_outputs <- function(disag_data,
  ...
  plot <- ggplot2::ggplot(fig_data, ggplot2::aes(x = ifelse(sex == "male", -mean, mean),
    ...
    ggplot2::theme(legend.position = "top", 
                   ...
                   plot.margin = margin(0.5, 0.3, 0.5, 0.3, "cm")) +
...
grid::grid.draw(cbind(ggplotGrob(p1), ggplotGrob(p2), ggplotGrob(p3)))
...
grid::grid.draw(cbind(ggplotGrob(p4), ggplotGrob(p5), ggplotGrob(p6)))
...
# Plot figs together
grid::grid.draw(cbind(ggplotGrob(p7), ggplotGrob(p8), ggplotGrob(p9)))
...
if("0" %in% area_levels) {
  ...
  p14 <- art_data %>% sf::st_drop_geometry() %>%
    ...
    ggtitle("Proportion PLHIV on ART") +
    coord_flip()
  ...
  p15 <- data %>% dplyr::filter(indicator == "incidence", 
    ...
    ggtitle("Rate of HIV infection") +
    coord_flip()
  
  # Plot figs together 
  grid::grid.draw(cbind(ggplotGrob(p13), ggplotGrob(p14), ggplotGrob(p15)))
...
x4 %>% gt::gt(rowname_col = "area_name") %>%
  ...
  gt::tab_spanner(
    ...
    columns = vars('plhiv', 'prevalence')) %>%
  gt::tab_spanner(
    label = gt::md("**Annual HIV infections**"),
    columns = vars('incidence', 'infections')) %>%
  gt::tab_spanner(
    label = gt::md("**Antiretroviral Treatment**"),
    columns = vars('art_coverage', 'art_num_residents', 'art_num_attend')) %>%
  gt::cols_label(
    plhiv = gt::md("**PLHIV**"),
